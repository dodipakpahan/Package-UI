...
import {
    ...
    insertUpdatePackageStep2,
    getPackageStep2ById,
    getPackageStep2Document,
    updateStep2DocumentStatus,
    deleteDocumentStep
...
export default function PackageStep2Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/sppbj.docx`;

        // Set the download attribute with the filename
        link.download = 'sppbj.docx';
    ...
    const handleDownloadBeritaAcara = () => {
        ...
        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_konfirmasi_dan_klarifikasi.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_konfirmasi_dan_klarifikasi.docx';
    ...
    const handleDownloadJaminanPelaksanaan = () => {
        // Create a link element
        const link = document.createElement('a');

        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/jaminan_pelaksanaan.docx`;

        // Set the download attribute with the filename
        link.download = 'jaminan_pelaksanaan.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep2(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep2ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep2ById(cookies.token, approveId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep2Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep2ById(cookies.token, stepDocumentId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep2DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 2);
            if (response === 0) {
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{
                                                    ...
                                                    <>
                                                        <div style={{
                                                            ...
                                                            <div style={{ display: "flex", flexDirection: "column" }}>
                                                                <div>Unduh Format BA Konfirmasi dan Klarifikasi Jaminan Pelaksanaan</div>
                                                                <Button style={{
                                                                    width: 150,
                                                                    height: 40
                                                                }} onClick={() => {
                                                ...
                                                }
                                                {
                                                    ...
                                                    <>
                                                        <div style={{
                                                            ...
                                                            <div style={{ display: "flex", flexDirection: "column", paddingBottom: 5 }}>
                                                                <div>Unduh Format Jaminan Pelaksanaan</div>
                                                                <Button style={{
                                                                    width: 150,
                                                                    height: 40
                                                                }} onClick={() => {
                                                                    handleDownloadJaminanPelaksanaan()
                                                                }}><div style={{ display: "flex", fontSize: 11, alignItems: "center" }}><FileWord size={20} /> Unduh Format Word</div> </Button>
                                                            </div>
                                                ...
                                                <div style={{
                                                    ...
                                                    <div hidden={cookies.userRole !== 2 && cookies.userRole !== 4} style={{
                                                        display: "flex",
                                                        justifyContent: "flex-end",
                                                        width: "100%",
                                                        paddingRight: 30
                                                    }}>
                                                        <Button hidden={cookies.userRole === 4 ? listDocument.length === 2 : (listDocument.length === 1 || listDocument.length > 2)} variant="primary" style={{
                                                            width: 130
                                                        }} onClick={() => {
                                                            setShowDocumentUploadModal(true);
                                                        }}>
                                                    ...
                                                    <Table  bordered hover>
                                                        <thead >
                                                            <tr>
                                                                ...
                                                                <th hidden={cookies.userRole !== 2 && cookies.userRole !== 4} style={{ width: 85, textAlign: "center", verticalAlign: 'middle' }}>Edit</th>
                                                                ...
                                                                <th style={{ width: 85, textAlign: "center", verticalAlign: "middle" }} hidden={cookies.userRole !== 2 && cookies.userRole !== 4}>Hapus</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {
                                                                listDocument.map((docs, index) => {
                                                                    return (
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td hidden={cookies.userRole !== 2 && cookies.userRole !== 4} style={{ textAlign: "center", verticalAlign: "top" }}>
                                                                            ...
                                                                            <td style={{ textAlign: "center", verticalAlign:"top" }} hidden={cookies.userRole !== 1}>
                                                                                <Button variant="success" hidden={!docs.document_status_name} style={{ width: 50 }} onClick={() => {
                                    ...
                                    {/* <Form.Group className="mb-3">
                                        ...
                                        <Form.Select onChange={(e) => {
                                            ...
                                            <option disabled></option>
                                            {
                                                ...
                                                <>
                                                    {
                                                        listDocument.length > 2 &&
                                                        <option value={0}>Berita Acara</option>
                                                    }

                                                    <option value={1}>SPPBJ</option>
                                                </>

                                            }
                                            {
                                                cookies.userRole == 4 &&
                                                <option value={2}>Jaminan Pelaksanaan</option>
                                            }
