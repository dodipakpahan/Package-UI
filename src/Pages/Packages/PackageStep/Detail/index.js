...
export default function PackageStepDetailPage() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/surat_undangan_review_laporan_hasil_pemilihan.docx`;

        // Set the download attribute with the filename
        link.download = 'surat_undangan_review_laporan_hasil_pemilihan.docx';
    ...
    const handleDownloadBeritaAcara = () => {
        ...
        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_reviu_laporan_hasil_pemilihan.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_reviu_laporan_hasil_pemilihan.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep1(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep1ById(cookies.token, downloadDocumentId);
    ...
    //     try {
    //         let response = await updatePackageDocumentStatus(cookies.token);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep1ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep1DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
            if (response.error_code === 0) {
                alert('Dokumen Telah Disetujui');
                loadDocumentData();
            } else {
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep1ById(cookies.token, stepDocumentId);
                                            ...
                                            <div style={{
                                                ...
                                                <div style={{
                                                    ...
                                                    <div hidden={cookies.userRole !== 2} style={{
                                                        ...
                                                        <Button disabled={listDocument.length > 0 && listDocument[0].document_status_name === "Menunggu Persetujuan"} hidden={listDocument.length > 1} variant="primary" style={{
                                                            width: 130
                                                        }} onClick={() => {
                                                            setShowDocumentUploadModal(true);
                                                        }}>
                                                    ...
                                                    <Table  bordered hover>
                                                        <thead >
                                                            <tr>
                                                                ...
                                                                <th style={{ width :200,textAlign: "center", verticalAlign: "middle" }}>Status</th>
