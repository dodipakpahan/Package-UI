...
export default function PackageStep9Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/undangan_rapat_persiapan_pelaksanaaan_kontrak.docx`;

        // Set the download attribute with the filename
        link.download = 'undangan_rapat_persiapan_pelaksanaaan_kontrak.docx';
    ...
    const handleDownloadBeritaAcara = () => {
        ...
        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_persiapan_pelaksanaan_kontrak.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_persiapan_pelaksanaan_kontrak.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep9(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep9ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep9Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep9ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep9ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep9DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
