...
export default function PackageStep16Page() {
    ...
    const handleDownloadMutu = () => {
        // Create a link element
        const link = document.createElement('a');

        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_rapat_mutual_check.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_rapat_mutual_check.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep16(cookies.token, newDocument, packageId, window.location.pathname);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep16ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep16Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep16ById(cookies.token, stepDocumentId);
 ...
 */}
                                                        ...
                                                        {/* <div>Unduh Template Dokumen Berita Acara Mutual Check</div>
                                                        <Button style={{
                                                            width: 150,
                                                            height: 40
                                                        }} onClick={() => {
