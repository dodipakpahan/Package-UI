...
import {
    ...
    insertUpdatePackageStep3,
    getPackageStep3ById,
    getPackageStep3Document,
    updateStep3DocumentStatus,
    deleteDocumentStep
...
export default function PackageStep3Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/surat_undangan_rapat_persiapan_penandatangan_kontrak.docx`;

        // Set the download attribute with the filename
        link.download = 'surat_undangan_rapat_persiapan_penandatangan_kontrak.docx';
    ...
    const handleDownloadBeritaAcara = () => {
        ...
        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_rapat_persiapan_penandatanganan_kontrak.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_rapat_persiapan_penandatanganan_kontrak.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep3(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep3ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep3Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep3ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep3ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep3DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 3);
            if (response === 0) {
                                                    ...
                                                    <>
                                                        <div style={{
                                                            ...
                                                            <div style={{ display: "flex", flexDirection: "column", paddingBottom: 5 }}>
                                                                <div>Unduh Format Undangan Rapat</div>
                                                                <Button style={{
                                                                    width: 150,
                                                                    height: 40
                                                                }} onClick={() => {
                                                                    ...
                                                                    return (
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td hidden={cookies.userRole !== 2} style={{ textAlign: "center", verticalAlign: "top" }}>
                                                                                <Button style={{ width: 50 }} disabled={docs.created_by !== cookies.userId || docs.document_status_name === "Disetujui"} onClick={() => {
                                                                                setNewDocument(docs)
