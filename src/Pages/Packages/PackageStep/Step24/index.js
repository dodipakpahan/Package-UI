...
export default function PackageStep24Page() {
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep24(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep24ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep24Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep24ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep24ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep24DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
                                            ...
                                            <div style={{
                                                ...
                                                <div style={{
                                                    ...
                                                    <div hidden={cookies.userRole !== 2 && cookies.userRole !== 4} style={{
                                                        ...
                                                        <Button hidden={cookies.userRole === 4 ? listDocument.length > 0 : listDocument.length === 0} variant="primary" style={{
                                                            width: 130
                                                        }} onClick={() => {
                                                            setShowDocumentUploadModal(true);
                                                        }}>
