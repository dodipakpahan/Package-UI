...
import {
    ...
    insertUpdatePackageStep14,
    getPackageStep14ById,
    getPackageStep14Document,
...
export default function PackageStep14Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_pembayaran.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_pembayaran.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep14(cookies.token, newDocument, packageId, window.location.pathname);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep14ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep14Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep14ById(cookies.token, stepDocumentId);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 14);
            if (response === 0) {
