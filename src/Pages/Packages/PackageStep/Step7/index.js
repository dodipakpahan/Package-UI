...
import {
    ...
    insertUpdatePackageStep7,
    getPackageStep7ById,
    getPackageStep7Document,
...
export default function PackageStep7Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/template_jaminan_uang_muka.docx`;

        // Set the download attribute with the filename
        link.download = 'template_jaminan_uang_muka.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep7(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep7ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep7Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep7ById(cookies.token, stepDocumentId);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 7);
            if (response === 0) {
