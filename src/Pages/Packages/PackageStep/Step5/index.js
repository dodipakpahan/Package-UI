...
export default function PackageStep5Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/undangan_penyerahan_lokasi_kerja.docx`;

        // Set the download attribute with the filename
        link.download = 'undangan_penyerahan_lokasi_kerja.docx';
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep5ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep5Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep5ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep5ById(cookies.token, approveId);
            console.log()
