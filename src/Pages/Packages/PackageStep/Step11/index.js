...
import {
    ...
    insertUpdatePackageStep11,
    getPackageStep11ById,
    getPackageStep11Document,
    updateStep11DocumentStatus,
    deleteDocumentStep
...
export default function PackageStep11Page() {
    ...
    const handleDownload = () => {
        ...
        link.href = `${process.env.PUBLIC_URL}/docs/undangan_rapat_pemeriksaan_bersama.docx`;

        // Set the download attribute with the filename
        link.download = 'undangan_rapat_pemeriksaan_bersama.docx';
    ...
    const handleDownloadBA = () => {
        ...
        // Set the href attribute to the path of the file in the public directory
        link.href = `${process.env.PUBLIC_URL}/docs/berita_acara_pemeriksaan_bersama.docx`;

        // Set the download attribute with the filename
        link.download = 'berita_acara_pemeriksaan_bersama.docx';
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep11(cookies.token, newDocument, packageId, window.location.pathname, detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep11ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep11Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep11ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep11ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep11DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 11);
            if (response === 0) {
