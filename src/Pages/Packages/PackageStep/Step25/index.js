...
import {
    ...
    insertUpdatePackageStep25,
    getPackageStep25ById,
    getPackageStep25Document,
    updateStep25DocumentStatus,
    deleteDocumentStep
...
export default function PackageStep25Page() {
    ...
    const uploadDocument = async () => {
        return new Promise(async (resolve, reject) => {
            try {
                let res = await insertUpdatePackageStep25(cookies.token, newDocument, packageId, window.location.pathname,detailPackage.account_type);
    ...
    const downloadData = async () => {
        try {
            let response = await getPackageStep25ById(cookies.token, downloadDocumentId);
    ...
    const loadDocumentData = async () => {
        try {
            ...
            let listDocument = await getPackageStep25Document(cookies.token, stepId);
    ...
    const loadDocumentById = async () => {
        try {
            let response = await getPackageStep25ById(cookies.token, stepDocumentId);
    ...
    const loadDocumentApproved = async () => {
        try {
            let response = await getPackageStep25ById(cookies.token, approveId);
    ...
    const ApproveDocument = async (documentId) => {
        try {
            ...
            let response = await updateStep25DocumentStatus(cookies.token, stepPayload, detailPackage.account_type);
    ...
    const removeDocument = async () => {
        try {
            let response = await deleteDocumentStep(cookies.token, removeId, 25);
            if (response === 0) {
                                                                ...
                                                                listDocument.map((docs, index) => {
                                                                    return (
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td style={{ textAlign: "center", verticalAlign:"top" }} hidden={cookies.userRole !== 1 || !docs.document_status_name}><Button variant="success" style={{ width: 50 }} onClick={() => {
